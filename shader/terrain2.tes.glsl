...
void main()
{
  const Entity entity = u_entities[ts_in[0].entityIndex];
  #include var_entity_model_matrix.glsl

  const Material material = u_materials[ts_in[0].materialIndex];
  sampler2D heightMap = sampler2D(u_texture_handles[material.heightMapTex]);

  float u = gl_TessCoord.x;
  float v = gl_TessCoord.y;

  vec2 t00 = ts_in[0].texCoord;
  vec2 t01 = ts_in[1].texCoord;
  vec2 t10 = ts_in[2].texCoord;
  vec2 t11 = ts_in[3].texCoord;
  ...
  ts_out.height = texture(heightMap, texCoord).y * 64.0 - 16.0;
  ...
  vec4 p = (p1 - p0) * v + p0 + normal * ts_out.height;
