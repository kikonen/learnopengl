...
namespace editor
{
    static int textEditCallbackStub(ImGuiInputTextCallbackData* data)
    {
        ConsoleFrame* console = (ConsoleFrame*)data->UserData;
        return console->textEditCallback(data);
    }
    ...
    void ConsoleFrame::renderInput()
    {
        ...
        {
            std::string cmd = m_state.m_inputBuffer;
            exec(cmd);
    ...
    ScriptResult ConsoleFrame::exec(const std::string& script)
    {
        //auto* lua  = m_scriptEngine->getLua();
        //if (!lua) return { false, "" };

        const auto& result = m_scriptEngine->execScript(script);
        if (!result.valid()) {
