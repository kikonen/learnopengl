...
namespace render {
    uint16_t BatchRegistry::getMultiDrawIndex(const MultiDrawKey& multiDraw)
    {
        ...
        auto index = static_cast<uint16_t>(m_multiDraws.size());
    ...
    uint16_t BatchRegistry::getCommandIndex(const CommandKey& cmd)
    {
        const auto& it = m_commandIndeces.find(cmd);
        if (it != m_commandIndeces.end()) return it->second;

        auto index = static_cast<uint16_t>(m_commands.size());
